    ...
    for (let i = 0; i < 3; i++) {
        ...
        for (let j = 0; j < 3; j++) {
            ...
            if (fields[index] === 'circle') {
             ...
             // Hervorhebung des aktuellen Spielers
             if (currentPlayer === fields[index]) {
                symbol = `<span class="highlight">${symbol}</span>`;
            }
...
function handleClick(cell, index) {
    // Überprüfen, ob das Spiel beendet ist, bevor ein Zug gemacht wird
    if (isGameFinished()) {
        return;
    }
